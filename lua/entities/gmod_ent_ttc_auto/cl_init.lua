...
local cv_part = CreateClientConVar("ttc_autotread_enable_particles", "1", true, false)
local cv_dt = CreateClientConVar("ttc_autotread_max_detail", "8", true, false, "maximum track render quality", 4, 16)
local cv_ad = CreateClientConVar("ttc_autotread_adaptive_detail", "1", true, false, "enhance track render quality as speed increases")
...
local waittime = 0
function ENT:_ENW2V_CHANGED(name, old, new)
	self.ttc_resetGearbox = CurTime() + waittime

	if not self.ttc_resetInitial then
		self.ttc_resetGearbox = CurTime() + 0.5
		self.ttc_resetInitial = true
	end

	if self._ENW2V_ONCHANGED then
		self:_ENW2V_ONCHANGED(name, old, new)
	end
end

function ENT:Think()
	if self.ttc_resetGearbox and self.ttc_resetGearbox < CurTime() then
		self.ttc_resetGearbox = nil
		self.ttc_vehicleMode = self:GetVehicleMode()
		self:setup_gearbox(self.ttc_vehicleMode)
		return
	end
	self:update_gearbox(self.ttc_vehicleMode)
end
...
function ENT:Draw()
	...
	if FrameTime() == 0 or gui.IsConsoleVisible() then -- or EyePos():DistToSqr(self:GetPos()) > 25000000 then
		self.ttc_isVisible = nil
		return
	end

	self:render_gearbox(self.ttc_vehicleMode)

	self.ttc_isVisible = true
end

function ENT:OnRemove()
	local snapshot = self.ttc_csents
...
function ENT:csents_create()
	if not self.ttc_csents then
		self.ttc_csents = {}
	end

	for k, v in pairs({"wheel"}) do
		if not self.ttc_csents[v] or not self.ttc_csents[v]:IsValid() then
			...
			CSEnt:SetLOD(0)
			self.ttc_csents[v] = CSEnt
		end
	end
end

function ENT:parentMatrix_Set()
	local p1 = self:GetParent()
	if p1 and p1:IsValid() then
		local p2 = p1:GetParent()
		if p2 and p2:IsValid() then
			p1 = p2
		end
	else
		p1 = self
	...
	local baseEnt = p1
	local forwardEnt = self:GetNW2Entity("ForwardEnt")

	if baseEnt == NULL then baseEnt = self end
	if forwardEnt == NULL then forwardEnt = self end

	local matrix = self.ttc_modelMatrix
	if not matrix then
		self.ttc_modelMatrix = Matrix()
		matrix = self.ttc_modelMatrix
	end
...
function ENT:parentMatrix_Get()
	return self.ttc_modelMatrix or self:GetWorldTransformMatrix()
end
...
function ENT:update_gearbox(vehicleMode)
	if not vehicleMode or not self.ttc_isVisible then return end
...
local function resetbounds(self)
	local rendermin, rendermax = Vector(0, 0, -self.ttc_wheels_groundClearance), Vector(0, 0, self.ttc_wheels_groundClearance*2)
	...
	for i = 1, self.ttc_wheels_count_ri do
		calcbounds(rendermin, rendermax, self.ttc_wheels_ri[i].posA + self.ttc_wheels_ri[i].info.bound1)
		calcbounds(rendermin, rendermax, self.ttc_wheels_le[i].posA + self.ttc_wheels_le[i].info.bound2)
	end
...
local function setup_modelinfo(self, vehicleMode, ttc_wheels_info)
	...
	for _, info in pairs(ttc_wheels_info) do
		...
		ttc_csents_wheel:SetModel(info.model)
		ttc_csents_wheel:DisableMatrix("RenderMultiply")
		ttc_csents_wheel:SetupBones()
		...
		local hmin, hmax, hbb = ttc_csents_wheel:GetHitBoxBounds(0, 0)
		if hmin and hmax then
			hbb = hmax -  hmin
		end

		local obb = ttc_csents_wheel:OBBMaxs() - ttc_csents_wheel:OBBMins()
		...
		local bodygroup = string.rep("0", ttc_csents_wheel:GetNumBodyGroups())
	...
	self.ttc_wheels_color = Vector(color.x/255, color.y/255, color.z/255)

	local material = string.Explode(", ", self:GetWheelMaterial())
	if #material == 1 then
		ttc_csents_wheel:SetMaterial(material[1])
		ttc_csents_wheel:SetSubMaterial(nil)
	else
		ttc_csents_wheel:SetMaterial(nil)
		for i = 1, #material, 2 do
			local id = tonumber(material[i])
			if id and material[i + 1] then
				ttc_csents_wheel:SetSubMaterial(id, material[i + 1])
			end
		end
	end
end
...
function ENT:setup_wheels(vehicleMode)
	self.ttc_wheels_info = {}
	local ttc_wheels_info = self.ttc_wheels_info

	self.ttc_wheels_ri = {}
	self.ttc_wheels_le = {}
	self.ttc_wheels_groundClearance = self:GetWheelOffsetZ()

	setup_modelinfo(self, vehicleMode, ttc_wheels_info)
	...
	if winfo then
		...
		setwheel(self.ttc_wheels_ri, 0, {trace = true, turn = 1, type = wtype, info = winfo, turn = 1, opp = 1, posL = pos + pos_ri, posA = pos + pos_ri, angL = Angle(0, 180, 0), angA = Angle(0, 180, 0)})

		local pos = Vector(wheelbase*winfo.offsetx, -winfo.offsety, winfo.trace_start)
		setwheel(self.ttc_wheels_le, 0, {trace = true, turn = 1, type = wtype, info = winfo, turn = 1, opp = 1, posL = pos + pos_le, posA = pos + pos_le, angL = Angle(0, 0, 0), angA = Angle(0, 0, 0)})
	...
	if winfo then
		...
		addwheel(self.ttc_wheels_ri, {type = wtype, info = winfo, opp = 1, posL = pos + pos_ri, posA = pos + pos_ri, angL = Angle(0, 180, 0), angA = Angle(0, 180, 0)})
	...
	if winfo then
		...
		for i = 0, num_road - 1 do
			...
			addwheel(self.ttc_wheels_ri, {trace = true, turn = turn, type = wtype, info = winfo, alt = alt, opp = opp, posL = pos + pos_ri, posA = pos + pos_ri, angL = Angle(0, alt and 0 or 180, 0), angA = Angle(0, alt and 0 or 180, 0)})
	...
	self.ttc_wheels_count_ri = #self.ttc_wheels_ri
	self.ttc_wheels_count_le = #self.ttc_wheels_le
	self.ttc_wheels_count_trace = wheeltracecount

	self.ttc_wheels_yaw = 0
	self.ttc_wheels_bias_fo = 0
	self.ttc_wheels_bias_ri = 0
	self.ttc_wheels_lastpos_ri = self:GetPos()
	self.ttc_wheels_lastpos_le = self:GetPos()
	self.ttc_wheels_lastrot_ri = 0
	self.ttc_wheels_lastrot_le = 0
	self.ttc_wheels_lastvel_ri = 0
	self.ttc_wheels_lastvel_le = 0

	self.ttc_tracks_textureres = self:GetTrackResolution()*self:GetTrackWidth()
	self.ttc_tracks_texturemap = 1/self.ttc_tracks_textureres
...
function ENT:update_wheels(vehicleMode)
	...
	local ttc_wheels_ri = self.ttc_wheels_ri
	local ttc_wheels_le = self.ttc_wheels_le
	...
	if enableTurning[vehicleMode] then
		local yaw = self:GetNW2Int("WheelYaw", 0)
		local rate = ft*5
		self.ttc_wheels_yaw = self.ttc_wheels_yaw + rate*(yaw - self.ttc_wheels_yaw)
		wheel_yaw = math_abs(self.ttc_wheels_yaw) > 0.01 and self.ttc_wheels_yaw or nil
	end

	local trace_length = self.ttc_wheels_groundClearance
	local trace_count = self.ttc_wheels_count_trace
	local trace_count_half = trace_count*0.5

	self.ttc_wheels_bias_fo = self.ttc_wheels_bias_fo + 5*ft*(self:GetNW2Float("Hydra", 0) - self.ttc_wheels_bias_fo)
	self.ttc_wheels_bias_ri = self.ttc_wheels_bias_ri + 5*ft*(self:GetNW2Float("HydraSide", 0) - self.ttc_wheels_bias_ri)

	local bias_fo = self.ttc_wheels_bias_fo
	if math_abs(bias_fo) < 0.01 then
		bias_fo = nil
	end

	local bias_ri = self.ttc_wheels_bias_ri*trace_length
	...
	for i = vehicleMode == "halftracked" and 0 or 1, self.ttc_wheels_count_ri do
		local wheel_ri = rawget(ttc_wheels_ri, i)
		local wheel_le = rawget(ttc_wheels_le, i)
...
function ENT:render_wheels(vehicleMode)
	local ttc_csents_wheel = self.ttc_csents.wheel
	if not ttc_csents_wheel or not ttc_csents_wheel:IsValid() then
		return
	end
	...
	local color = self.ttc_wheels_color
	...
	for i = vehicleMode == "halftracked" and 0 or 1, self.ttc_wheels_count_ri do
		...
		if prevtype ~= wheel_ri.type or prevalt ~= wheel_ri.alt then
			...
			if prevmodel ~= wheel_info.model then
				...
				ttc_csents_wheel:SetModel(prevmodel)
			end

			if prevbody ~= wheel_info.bodygroup then
				prevbody = wheel_info.bodygroup
				ttc_csents_wheel:SetBodyGroups(prevbody)
			end
			...
			ttc_csents_wheel:EnableMatrix("RenderMultiply", prevalt and wheel_info.scale2 or wheel_info.scale1)
		end

		local wpos, wang = LocalToWorld(wheel_ri.posL, wheel_ri.angL, pos, ang)
		ttc_csents_wheel:SetAngles(wang)
		ttc_csents_wheel:SetPos(wpos)
		ttc_csents_wheel:SetupBones()
		ttc_csents_wheel:DrawModel()
...
local function createTrackTexture(path)
	local folders = { "tanktrack_controller_new/" }
	local diffuse
...
function ENT:setup_tracks(vehicleMode)
	...
	self.ttc_tracks_color = Vector(color.x/255, color.y/255, color.z/255)

	self.ttc_tracks_texture = createTrackTexture(self:GetTrackMaterial())
	self.ttc_tracks_tension = 1 - self:GetTrackTension()
	self.ttc_tracks_tensor  = Vector(0, 0, -self.ttc_tracks_tension*3)

	self.ttc_tracks_model = trackverts_model(self:GetTrackWidth(), self:GetTrackHeight())
	self.ttc_tracks_modelcount = #self.ttc_tracks_model

	self.ttc_tracks_nodes_ri = {}
	self.ttc_tracks_nodes_le = {}
	self.ttc_tracks_nodescount_ri = 0
	self.ttc_tracks_nodescount_le = 0

	self.ttc_tracks_verts_ri = {}
	self.ttc_tracks_verts_le = {}
	self.ttc_tracks_vertscount_ri = 0
	self.ttc_tracks_vertscount_le = 0

	self.ttc_tracks_normals_ri = {}
	self.ttc_tracks_normals_le = {}

	local trackHeight = self:GetTrackHeight()*0.5
	for i = 1, self.ttc_wheels_count_ri do
		local trackRadius = self.ttc_wheels_ri[i].info.radius + trackHeight
		self.ttc_wheels_ri[i].trackRadius = trackRadius
		self.ttc_wheels_le[i].trackRadius = trackRadius
		self.ttc_wheels_ri[i].trackHeight = trackRadius + trackHeight + 0.1
		self.ttc_wheels_le[i].trackHeight = trackRadius + trackHeight + 0.1
	end

	self:update_wheels(vehicleMode)

	self.ttc_tracks_ready = nil
end
...
function ENT:update_tracks(vehicleMode)
	...
	local tension_val = self.ttc_tracks_tension
	local tension_dir = self.ttc_tracks_tensor

	for i = 1, 2 do
		local trackroots, trackrootscount, tracknodes, tracknodesdetail

		if i == 1 then
			trackroots = self.ttc_wheels_ri
			trackrootscount = self.ttc_wheels_count_ri
			tracknodes = self.ttc_tracks_nodes_ri
			tracknodesdetail = adaptive_detail and (min_detail + math_floor(math_abs(self.ttc_wheels_lastvel_ri)*2)) or max_detail
		else
			trackroots = self.ttc_wheels_le
			trackrootscount = self.ttc_wheels_count_le
			tracknodes = self.ttc_tracks_nodes_le
			tracknodesdetail = adaptive_detail and (min_detail + math_floor(math_abs(self.ttc_wheels_lastvel_le)*2)) or max_detail
		end
		...
		if i == 1 then self.ttc_tracks_nodescount_ri = tracknodescount else self.ttc_tracks_nodescount_le = tracknodescount end
	end

	local trackmodel = self.ttc_tracks_model
	local trackmodelcount = self.ttc_tracks_modelcount
	local trackresolution = self.ttc_tracks_texturemap
	...
	for i = 1, 2 do
		...
		if i == 1 then
			tracknodes = self.ttc_tracks_nodes_ri
			tracknodescount = self.ttc_tracks_nodescount_ri
			trackverts = self.ttc_tracks_verts_ri
			tracknormals = self.ttc_tracks_normals_ri
		else
			tracknodes = self.ttc_tracks_nodes_le
			tracknodescount = self.ttc_tracks_nodescount_le
			trackverts = self.ttc_tracks_verts_le
			tracknormals = self.ttc_tracks_normals_le
		end
		...
		if i == 1 then self.ttc_tracks_vertscount_ri = trackvertscount else self.ttc_tracks_vertscount_le = trackvertscount end
	end

	self.ttc_tracks_ready = true
end
...
function ENT:render_tracks(vehicleMode)
	if not self.ttc_tracks_ready then return end

	cam.PushModelMatrix(self:parentMatrix_Get())

	local texture = self.ttc_tracks_texture
	if texture and self.ttc_tracks_color then
		texture:SetVector("$color2", self.ttc_tracks_color)
	end
	...
	for i = 1, 2 do
		...
		if i == 1 then
			...
			tracknodescount = self.ttc_tracks_nodescount_ri
			trackverts = self.ttc_tracks_verts_ri
			trackvertscount = self.ttc_tracks_vertscount_ri
			tracknormals = self.ttc_tracks_normals_ri
			trackscroll = self.ttc_wheels_lastrot_ri
		else
			tracknodes = self.ttc_tracks_nodes_le
			tracknodescount = self.ttc_tracks_nodescount_le
			trackverts = self.ttc_tracks_verts_le
			trackvertscount = self.ttc_tracks_vertscount_le
			tracknormals = self.ttc_tracks_normals_le
			trackscroll = self.ttc_wheels_lastrot_le
		end

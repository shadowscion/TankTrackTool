...
function mode:setup(ent, isdouble)
	...
	for i = 1, values.wheelCount do
		...
		ttlib.fixWheel(wheel.csent, model, bodygroup, material, radius, width)
		...
		if wheel.suspension then
			...
			local pairgap = ttlib.map(values.suspensionPairGap, -1, 1, -0.5, 0.5)
	...
	for i = 1, values.rollerCount do
		...
		ttlib.fixWheel(roller.csent, model, bodygroup, material, radius, width)

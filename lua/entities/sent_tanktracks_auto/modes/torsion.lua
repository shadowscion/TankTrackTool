...
local ttlib = ttlib
local sin, atan, icos = ttlib.sin, ttlib.atan, ttlib.icos

local LocalToWorld, WorldToLocal = LocalToWorld, WorldToLocal
local math = math

//
local mode = ttlib.mode("torsion")
...
function mode:setup(ent, isdouble)
	ttlib.modes.classic.setup(self, ent, ent.ttdata_isdouble)
	...
	for i = 1, values.wheelCount do
		...
		arm.material = ttlib.isValidMaterial(values.suspensionMaterial)
		...
		if arm.type == 1 then
			...
			arm.model = ttlib.isValidModel("models/tanktracktool/torsionbar1.mdl")
			...
			arm.model = ttlib.isValidModel("models/tanktracktool/bogie1.mdl")
		...
		if values.suspensionTDamp ~= 0 then
			if part.id % values.suspensionTDamp == 0 then
				...
				damper.model = ttlib.isValidModel("models/tanktracktool/spring1.mdl")
...
//
local mode = ttlib.mode("bogie")
mode.csents = {wheel = true, roller = true, arm = true, damper = true, bogie = true}

function mode:setup(ent, isdouble)
	return ttlib.modes.torsion.setup(self, ent, isdouble)
end
function mode:think(ent, isdouble)
	return ttlib.modes.torsion.think(self, ent, isdouble)
end

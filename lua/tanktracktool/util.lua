...
// MATH
function ttlib.sin(a)
	return math.sin(a*deg2rad)
end
function ttlib.atan(a, b)
	return math.atan2(a, b)*rad2deg
end
function ttlib.acos(a)
	return math.acos(a)*rad2deg
end
local acos = ttlib.acos
function ttlib.icos(a, b, c)
	return acos((a^2 + b^2 - c^2) / (2*a*b))
end
function ttlib.lerp(a, b, t)
	return a + (b - a)*t
end
function ttlib.map(x, in_min, in_max, out_min, out_max)
    return (x - in_min)*(out_max - out_min)/(in_max - in_min) + out_min
end
function ttlib.calcbounds(min, max, pos)
...
function ttlib.renderbounds(self)
	...
	ttlib.calcbounds(rendermin, rendermax, renderpos)

	for i = 1, #self.ttdata_parts do
		local wheel = self.ttdata_parts[i][1]
		ttlib.calcbounds(rendermin, rendermax, wheel[1] + wheel.maxs)
		if self.ttdata_isdouble then
			ttlib.calcbounds(rendermin, rendermax, wheel[2] + wheel.mins)
		end
	end
...
ttlib.rendercomponent = rendercomponent
...
function ttlib.mode(name)
	ttlib.modes[name] = { name = name, setup = function() end, think = function() end, addpart = addpart, render = rendercsents, createcsents = createcsents, csents = {} }
	return ttlib.modes[name]
end
...
ttlib.isValidMaterial = IsValidMaterial
...
ttlib.isValidModel = IsValidModel


// SCALE FIXER
function ttlib.fixWheel(csent, model, bodygroups, materials, radius, width)

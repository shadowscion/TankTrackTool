...
do
	local tblRow = vgui.RegisterTable({
		...
		Setup = function(self, var, edit)
			...
			local rowprop = "enw2vprop_" .. rowtype
			...
			if not IsValid(self.Inner) then
				self.Inner = self.Container:Add("enw2vprop_Generic")
			end
	...
	derma.DefineControl("enw2v_properties", "", PANEL, "Panel")
end
...
do
	...
	function PANEL:RebuildControls()
		...
		if not self.m_Entity._ENW2V_EDITOR then return end

		self.hidepanels = {}

		local editor = self.m_Entity:_ENW2V_EDITOR()
		...
		for k, v in pairs(self.hidepanels) do
			SetVisible(self, k, self.m_Entity:_ENW2V_GET(k))
		end
	end
	...
	function PANEL:EditVariable(vname, var, edit, editor)
		...
		row.DataUpdate = function(_)
			...
			row:SetValue(self.m_Entity:_ENW2V_GET(vname))
		end

		row.DataChanged = function(_, val)
			if not IsValid(self.m_Entity) then self:EntityLost() return end
			self.m_Entity:_ENW2V_SET(vname, tostring(val))
			SetVisible(self, vname, tostring(val))
		end
	end

	derma.DefineControl("enw2v_editor", "", PANEL, "enw2v_properties")
end
